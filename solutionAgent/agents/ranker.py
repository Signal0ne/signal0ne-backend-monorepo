...
class ChatAgent:
    ...
    def gen_ques(self, outputs, logs):
        """Generate questions from the logs."""
        prompt = f"""You are a helpful assistant that helps ranking the outputs of websearch based on how relevant they are to solving the error recieved in logs.
        Here are user logs: {logs}\n
        Here are the websearch results: {outputs}\n
        Only return the index of the most relevant outputs.
        Output format is {{"ranks":[1,2,4,.....],"reason":}}.\n
        Json:"""
        i=0
        while i<3:
            try:
...
if __name__ == "__main__":
    ...
    ranks = chat_agent.gen_ques(outputs, logs)
    print(ranks)